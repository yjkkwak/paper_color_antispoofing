...
from networks import getpatchnetresnet18
...
def trainepoch(epoch, trainloader, model, mymetric, criterion, optimizer, averagemetermap):
  ...
  for index, item in enumerate(trainloader):
    ...
    fbtimer.tic()
    imgp1, imgp2, labels = imgp1.cuda(), imgp2.cuda(), labels.cuda()

    """
    imgp1_logit, imgp2_logit = model(imgp1, imgp2)
