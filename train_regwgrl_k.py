...
from test import testmodel, testmodelallimgs
...
parser.add_argument('--lk', type=int, default=11, help='level K')
...
struuid = "{}_{}_{}_bsize{}_opt{}_lr{}_gamma_{}_epochs_{}_meta_{}_seed_{}_k_{}".format(getbasenamewoext(os.path.basename(args.lmdbpath)),
...
def trainepoch(epoch, trainloader, model, criterion, optimizer, averagemetermap):
  ...
  regrsteps = torch.linspace(0, 1.0, steps=args.lk).cuda()
...
def trainmodel():
  ...
  mynet = getbaseresnet18wgrl(numclasses=args.lk)
  # mynet = getresnet1s8()
  mynet = mynet.cuda()
  ...
  traindataset = lmdbDatasetwmixup(args.lmdbpath, transforms, args.lk)
