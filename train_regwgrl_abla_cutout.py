...
def trainepoch(epoch, trainloader, model, criterion, optimizer, averagemetermap):
  ...
  for index, (tmpimages, tmplabels, imgpath, rimg, rlab, tmpuid1, tmpuid2) in enumerate(trainloader):
    fbtimer.tic()
    images, labels = tmpimages.cuda(), tmplabels.cuda()
    images = cutout(images.clone())
    uid1 = tmpuid1.cuda()
    optimizer.zero_grad()
    logit, dislogit = model(images)
    cutoutloss = criterion["cls"](logit, labels)
    advclsloss = criterion["cls"](dislogit, uid1)
    loss = cutoutloss + advclsloss
...
def trainmodel():
  ...
  for epoch in range(startepoch, args.epochs):
    ...
    logger.print (strprint)
    scheduler.step()
    #if averagemetermap["acc_am"].avg > 98.0:
    if averagemetermap["acc_am"].avg > 93.0:
