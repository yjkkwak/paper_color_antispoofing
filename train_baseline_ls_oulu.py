...
from augs.cutmix import cutmix_data, mixup_criterion, cutout
...
def trainepoch(epoch, trainloader, model, criterion, optimizer, averagemetermap):
  ...
  for index, (tmpimages, tmplabels, imgpath, rimg, rlab, tmpuid1, tmpuid2) in enumerate(trainloader):
    ...
    loss = criterion["ls"](logit, labels)
    #loss = mixup_criterion(criterion["cls"], logit, mixlabel[0], mixlabel[1], mixlabel[2]).mean()
...
def trainmodel():
  ...
  mynet = getbaseresnet18wgrl(numclasses=2)
  # mynet = getbaseresnet18(numclasses=2)
  ...
  criterion["ls"] = LabelSmoothingLoss().cuda()
  #
  if args.opt.lower() == "adam":
    # works
    #optimizer = optim.Adam(mynet.parameters(), lr=args.lr, weight_decay=5e-4)
    optimizer = optim.Adam(mynet.parameters(), lr=args.lr, weight_decay=2e-5)#bmvc2021
  else:
